...
class KnowledgeGraph:
    ...
    def attribute_triples(self, entity: Entity = None) -> Set[AttributeTriple]:
        ...
        triples = set()
        for node in nodes:
            triples.update(set([(node[0], attr, val) for attr, val in node[1].items()]))
        return triples

    def relation_quadruples(self, head_entity: Entity = None, 
            tail_entity: Entity = None) -> List[RelationQuadruple]:
        if head_entity and tail_entity:
            all_quads_from_head = list(self.mdg.edges(head_entity, data=True, keys=True))
            quads = [quad for quad in all_quads_from_head if quad[1] == tail_entity]
        elif head_entity:
            quads = list(self.mdg.edges(head_entity, data=True, keys=True))
        else:
            quads = list(self.mdg.edges(data=True, keys=True))
        quads = [(src_entity, relation, dest_entity, attributes) 
